...
void bezierCurve (QPointF *det, int SR, QPointF *p, int length) {
    double t = 1.0/SR;
    double tn = 0.0;
    double distance[3];
    double w[length];
    ...
    */
    ...
    //memset(&det, 0, sizeof(QPointF)*length);
...
void triangleDist (QPointF *p, double *dists) {
    int i;
    for (i = 0; i < 3; i++) {
        if (p[i].x() != p[(i*i+2)%3].x() || p[i].y() != p[(i+1)%3].y()) {
